...
def run_f16_sim(initial_state, tmax, ap, step=1/30, extended_states=False, model_str='morelli',
    ...
    # TODO(yue)
    epsilon=1e-8
    grad, original_f = get_grad(der_func, times[-1], states[-1], epsilon)
    grad_list = [grad]
    original_f_list = [original_f]
    ...
    while integrator.status == 'running':
        ...
        if integrator.t >= times[-1] + step:
            ...
            while integrator.t >= times[-1] + step:
                ...
                original_f_list.append(original_f)
    ...
    # TODO(yue)
    res['grad_list'] = grad_list

    res["original_f"] = original_f_list
...
# TODO(yue)
def get_grad(der_func, t, state, epsilon):
    original_f = der_func(t, state)

    # print("state.shape",state.shape)
    ndim = state.shape[0]
    grads=[]
    # print("p=",state[6])  # TODO(debug)
    for i in range(ndim):
        new_state0 = np.array(state)
        new_state0[i] -= epsilon
        rv0 = der_func(t, new_state0)

        new_state1 = np.array(state)
        new_state1[i] += epsilon
        rv1 = der_func(t, new_state1)

        grad = (rv1 - rv0 ) / 2 / epsilon
        grads.append(grad)

        # TODO(debug)
        # if i==6:
        #     print("i=6, summary:",new_state0[i], new_state1[i], rv0[i], rv1[i])
        # print("i=",i,", summary:", new_state0[i], new_state1[i], rv0[i], rv1[i])
        # print("i=",i,grad[i])

    grads = np.array(grads)
    # print(np.max(np.diagonal(grads)), np.min(np.diagonal(grads)))  # TODO(debug)

    return grads, original_f